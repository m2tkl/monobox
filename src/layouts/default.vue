<template>
  <div>
    ...
    <div class="h-screen w-full flex pt-[30px]">
      <!-- Sidebar -->
      <aside class="sidebar-area h-[calc(100vh-30px)] overflow-auto"
        :class="{ 'w-[250px] flex-shrink-0': ui.isSidebarOpen, 'w-0 overflow-hidden': !ui.isSidebarOpen }">
        <SidebarMenu :isOpen="ui.isSidebarOpen" />
      </aside>

      <!-- Main content -->
      <div class="flex-1 h-full flex w-[calc(100%-254px)]" :class="{ 'justify-center w-full': !ui.isSidebarOpen }">
        <div :class="{ 'max-w-7xl': !ui.isSidebarOpen }" class="h-full w-full">
            ...
            <div v-if="$route.params.workspace" class="flex items-center w-full">
              <div class="flex items-center gap-2.5 w-full">
                <h1 class="font-mono text-xl text-slate-500 gap-1.5 flex items-center w-full">
                  <USkeleton v-if="!wspace" class="h-6 w-[250px]" />
                  <NuxtLink v-else :to="`/${$route.params.workspace}`" class="flex items-center">
                    <UIcon :name="iconKey.home" class="h-6 w-6" />
                  </NuxtLink>
